...
class XiaohongshuAgent:
    ...
    async def setup_browser(self) -> None:
        ...
        headless = self.browser_config.get(
            "headless", optimized_config.get("headless", False)
        )
        window_w = self.browser_config.get(
            "window_width", optimized_config.get("window_width", 1280)
        )
        window_h = self.browser_config.get(
            "window_height", optimized_config.get("window_height", 720)
        )
        ...
        for item in scan_dir.iterdir():
            if item.is_dir():
                ...
                if post_data:
                    ...
                    logger.info(
                        f"  ✅ 找到内容: {title} ({images}张图片, {text_len}字)"
                    )
    ...
    def _scan_post_directory(self, dir_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if post_data["images"]:
                ...
                logger.info(
                    f"📸 帖子 '{post_data['title']}' 使用第一张图片: {first_image['name']}"
                )
    ...
    async def _load_cookies(self) -> bool:
        ...
        try:
            ...
            if self.cookie_file_path and os.path.exists(self.cookie_file_path):
                ...
                cookies = self.cookie_manager.load_cookies_from_file(
            ...
            if cookies:
                # 设置cookies到浏览器
                success = await self.cookie_manager.set_browser_cookies(
                    self.browser_context, cookies
                )
                if success:
    ...
    async def _verify_cookie_login(self) -> bool:
        ...
        try:
            ...
            if not page:
                await self.browser_context.navigate_to(
    ...
    def extract_title_and_description(self, text: str) -> tuple[str, str]:
        """从文案中解析标题与正文。
        优先识别以 'title' 和 'description'（支持中英文冒号、大小写）标识的段落；
        若未标识，则取首行作为标题、其余为正文。
        """
        if not text:
            return ("今天的分享 ✨", "")
        ...
        for raw_line in text.splitlines():
            ...
            # 匹配标题标识
            if re.match(r"^(title|标题)\s*[:：]", line, flags=re.IGNORECASE):
                value = re.sub(
                    r"^(title|标题)\s*[:：]\s*", "", line, flags=re.IGNORECASE
                )
                if value:
            ...
            # 匹配正文标识
            if re.match(r"^(description|正文|内容)\s*[:：]", line, flags=re.IGNORECASE):
                value = re.sub(
                    r"^(description|正文|内容)\s*[:：]\s*",
                    "",
                    line,
                    flags=re.IGNORECASE,
                )
            ...
            if current == "title":
                ...
                # 尚未遇到标识：先积累到正文，稍后首行兜底为标题
                desc_lines.append(line)

        title = " ".join([t for t in title_lines if t is not None]).strip()
        description = "\n".join(desc_lines).strip()

        # 兜底：若未显式给出标题，则用正文首行作为标题
        if not title:
            first_line = (
                description.splitlines()[0].strip() if description else "今天的分享 ✨"
            )
            title = first_line
            # 去掉正文中首行，避免重复
    ...
    async def post_to_xiaohongshu(self, post_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"内容长度: {len(content)}, 图片数量: {len(images)} (已简化为单图)"
            )
            ...
            if images:
                ...
                # 有图片的发布流程
                # 从文案中拆分出标题与正文
                title_text, description_text = self.extract_title_and_description(
                    content
                )
                ...
                6. **填写内容**：
                   - 仅将“标题”内容填入标题输入框（通常标注“标题”），不要把“正文”内容放入标题
                   - 如果标题长度超出平台限制，请在不改变核心含义的前提下自动精简标题
                   - 将“正文”完整填入描述/正文输入框（包含换行与标签）
                7. **重要**：不要点击“话题”或“添加话题”按钮
                8. 直接点击“发布”按钮完成发布

                成功标准：
                - 成功上传图片并进入编辑界面（能看到标题与正文输入框）
                - 标题栏仅含标题文本、不包含正文
                - 正文栏包含完整正文
                - 成功点击发布按钮，并看到发布成功提示或 URL 包含 published=true
                ...
                **关键问题处理指南**：
                ...
                return {
                    ...
                    "analysis": {"decision_reason": "小红书平台限制：不支持纯文字发布"},
                }
            ...
            is_success = any(
                indicator in final_result_str for indicator in success_indicators
            )
            ...
            # 检查失败指标
            has_failure = any(
                indicator in final_result_str for indicator in failure_indicators
            )

            # 🔧 新增：检查是否需要重试
            needs_retry = any(
                indicator in final_result_str for indicator in retry_indicators
            )
            ...
            return {
                ...
                "error": str(e),
                "post_title": post_data.get("title", "未知"),
                "analysis": {"decision_reason": "发生异常错误"},
            }

    @time_execution_async("--run (xiaohongshu_agent)")
    async def run_posting_task(
        self, max_posts: int = 5, status_callback=None
    ) -> List[Dict[str, Any]]:
        """
        运行小红书发帖任务
        ...
        try:
            ...
            await update_status(
                f"📝 准备发布 {len(posts_to_publish)} 条内容",
                {"total_posts": len(posts_to_publish), "current_post": 0},
            )
            ...
            for i, post_data in enumerate(posts_to_publish, 1):
                ...
                if self.stop_requested:
                    logger.info(
                        f"🛑 接收到停止信号，已完成 {post_count}/{len(posts_to_publish)} 条内容"
                    )
                    break
                ...
                if current_task and current_task.cancelled():
                    logger.info(
                        f"🛑 任务被取消，已完成 {post_count}/{len(posts_to_publish)} 条内容"
                    )
                ...
                # 实时更新当前发布状态
                await update_status(
                    f"📤 正在发布第 {i}/{len(posts_to_publish)} 篇帖子",
                    {
                        "current_post": i,
                        "total_posts": len(posts_to_publish),
                        "post_title": title,
                ...
                try:
                    ...
                    while retry_count <= max_retries:
                        ...
                        # 检查是否需要重试
                        if (
                            result.get("retry_needed", False)
                            and retry_count < max_retries
                        ):
                            retry_count += 1
                            logger.warning(
                                f"⚠️ 发布遇到可重试错误，准备第 {retry_count} 次重试: {result.get('error', '')}"
                            )
                            continue
                        else:
                            # 不需要重试或已达到最大重试次数
                    ...
                    if result["success"]:
                        ...
                        retry_msg = (
                            f" (经过 {retry_count} 次重试)" if retry_count > 0 else ""
                        )
                        logger.info(f"✅ 第 {i} 篇帖子发布成功: {title}{retry_msg}")

                        # 实时更新成功状态
                        await update_status(
                            f"✅ 第 {i} 篇帖子发布成功: {title}{retry_msg}",
                            {
                                "current_post": i,
                                "total_posts": len(posts_to_publish),
                                "post_title": title,
                        ...
                        if i < len(posts_to_publish) and not self.stop_requested:
                            ...
                            for wait_sec in range(wait_time):
                                ...
                                # 实时更新等待状态
                                await update_status(
                                    f"⏱️ 发布失败，等待 {wait_time - wait_sec} 秒后重试",
                                    {
                                        "current_post": i,
                                        "total_posts": len(posts_to_publish),
                                        "post_title": title,
            ...
            if self.stop_requested:
                logger.info(
                    f"🛑 任务被中断，已完成 {successful_posts}/{total_attempts} 条内容"
                )
                await update_status(
                    "🛑 任务被中断",
                    {
                ...
                logger.info(
                    f"🎉 发帖任务完成！成功发布 {successful_posts}/{total_attempts} 条内容"
                )
                await update_status(
                    "🎉 发帖任务完成！",
                    {
